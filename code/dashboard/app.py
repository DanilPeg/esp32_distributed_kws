...
@app.get("/api/wire_snapshot")
async def api_wire_snapshot() -> JSONResponse:
    """Snapshot in the on-board master's WebSocket wire format. Lets the
    FastAPI dashboard template reuse the master's UI code verbatim."""
    snapshot = await asyncio.to_thread(loaders.build_wire_snapshot)
    return JSONResponse(snapshot)


@app.get("/api/wire_stream")
async def api_wire_stream(request: Request) -> StreamingResponse:
    """SSE stream of wire-format snapshots, one per second. UI subscribes
    via EventSource('/api/wire_stream') and renders identically to the
    on-board master dashboard."""
    interval_sec = 1.0
        ...
        try:
            while True:
                if await request.is_disconnected():
                    break
                snap = await asyncio.to_thread(loaders.build_wire_snapshot)
