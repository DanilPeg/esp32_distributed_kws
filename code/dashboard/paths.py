...
# Camera-link events (master → camera trigger, camera → master reply) are
# written here by hash_kws_master_demux_bridge.py when it sees
# `hash_evt kind=cam_trigger ...` / `kind=cam_reply ...` lines on the
# master's Serial. Optional — the dashboard renders an empty camera card
# until the first event lands.
CAMERA_DIR: Path = JOURNAL_DIR / "hash_kws_camera"
CAMERA_EVENTS: Path = CAMERA_DIR / "events.jsonl"
