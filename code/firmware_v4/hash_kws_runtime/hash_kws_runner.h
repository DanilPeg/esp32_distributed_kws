...
class HashKwsRunner {
 public:
  ...
  // v4 wire-fix: pass a non-null `out_logits` (float, >= num_classes) to also
  // receive the TRUE classifier logits. `output_scores` stays the
  // centered-softmax int8 the node uses internally for top1/score/gating; the
  // ESP-NOW wire carries the logits (quantised by the caller) so the master
  // aggregator can run temperature_scaled / learned_weights correctly.
  bool Invoke(const int8_t* model_input,
              int8_t* scratch_a,
              int8_t* scratch_b,
              int8_t* output_scores,
              float* out_logits = nullptr) const;
