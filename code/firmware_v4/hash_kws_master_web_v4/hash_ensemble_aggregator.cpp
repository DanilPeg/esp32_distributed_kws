...
// v4 wire-fix: the wire now carries TRUE logits quantised as int8 = round(logit/0.25)
// (see hash_kws_runner.cpp + micro_speech.ino). Dequantise on read so the mean /
// temperature / learned modes operate in the real logit domain (matches the
// training-side aggregation.py and the laptop stand).
constexpr float kWireDequant = 0.25f;

// Convert int8 wire logits -> float logit view, scale by 1/T if requested.
void load_logits(const Vote& v, uint8_t num_classes, const float scale, float* out) {
  for (uint8_t c = 0; c < num_classes; ++c) {
    out[c] = static_cast<float>(v.logits[c]) * kWireDequant * scale;
  }
}
  ...
  if (effective_mode == Mode::kModeTemperatureScaled) {
    ...
    for (uint8_t i = 0; i < num_nodes_; ++i) {
      ...
      for (uint8_t s = 0; s < ring.count; ++s) {
        ...
        if (!slot_in_window(v, now_ms, window_ms_)) continue;
        for (uint8_t c = 0; c < num_classes_; ++c) {
          per_node_mean[i][c] += kWireDequant * static_cast<float>(v.logits[c]);
        }
      }
  ...
  if (effective_mode == Mode::kModeMeanLogits) {
    ...
    for (uint8_t i = 0; i < num_nodes_; ++i) {
      ...
      for (uint8_t s = 0; s < ring.count; ++s) {
        ...
        if (!slot_in_window(v, now_ms, window_ms_)) continue;
        for (uint8_t c = 0; c < num_classes_; ++c) {
          aggregated[c] += kWireDequant * static_cast<float>(v.logits[c]);
        }
        ++n_used;
      }
    }
