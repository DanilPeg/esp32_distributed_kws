...
// Bias added to noise classes (unknown / silence) before picking top1.
// Suppresses false positives on real keywords in low-confidence states.
// v4 wire-fix: the aggregator now works in the TRUE logit domain (wire int8
// dequantised by 0.25; logits span ~[-25,+8]), so this is in logit units.
// 24 (the old centered-softmax-domain value, range +-128) would swamp every
// keyword and the master would only ever emit unknown/silence. ~0.5 matches
// the laptop stand. Set to 0 to disable. Useful range ~0.3..1.5 (logit units).
#define HASH_KWS_AGG_NOISE_BOOST 0.5f
#endif
