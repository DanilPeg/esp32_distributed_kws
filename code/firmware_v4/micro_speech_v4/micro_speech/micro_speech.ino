...
float  hash_logits_f[hash_kws::kCategoryCount];      // v4 wire-fix: true logits from runner
int8_t hash_wire_logits[hash_kws::kCategoryCount];   // v4 wire-fix: quantised logits for ESP-NOW
...
void loop() {
  ...
  if (!hash_runner->Invoke(hash_model_input_buffer, hash_scratch_a,
                           hash_scratch_b, hash_output_scores, hash_logits_f)) {
  ...
  EmitHashInferEvent(current_time, how_many_new_slices, recent_slice_max,
                     ...
                     top3_score_u8);
#if HASH_KWS_USE_ESPNOW
  // v4 wire-fix: put TRUE logits (quantised, logit ~= q*0.25; must match master
  // dequant) on the wire instead of the centered-softmax. Per-class logits span
  // ~[-25, +8]; x4 maps to [-99, +32] (no clipping), clamped to +-127.
  for (int i = 0; i < hash_kws::kCategoryCount; ++i) {
    int q = static_cast<int>(lroundf(hash_logits_f[i] * 4.0f));
    if (q < -127) q = -127;
    if (q > 127) q = 127;
    hash_wire_logits[i] = static_cast<int8_t>(q);
  }
