# Context Pack
Generated: 2026-02-24T20:09:03Z
Query: Daily review: Training + analysis
...
- Custom task: `/task <title> | <description>` (enqueue a custom agent task).
- Merge branch: `/merge <branch> [--force]` (merge a task branch; only on user initiation).
...
- For changes to agent workflow/prompt: define a small eval checklist, run it, and log results.
 ...
 - For code edits: use a per-task git branch when the repo is a git worktree. Do not merge automatically; merges only on user initiation. If git is not initialized, log and skip branch creation.
...
v1.23 (2026-02-24): added /task command and git-branch rule for code edits.
...
File: C:\Users\Danil\diploma_esp32_distributed_nn\notes\Journal\2026-02-24_agent_codex_1500cd2d.yaml
date: 2026-02-24
session_id: agent_codex_1500cd2d
goal: Review firmware core (I2S/audio, inference loop, logging)
inputs:
  - code/firmware/i2s_mic_test/i2s_mic_test.ino
  - code/firmware/micro_speech_sim/README.md
  - code/firmware/micro_speech_sim/micro_speech/audio_provider.cpp
  - code/firmware/micro_speech_sim/micro_speech/micro_speech.ino
  - code/firmware/micro_speech_sim/micro_speech/feature_provider.cpp
  - code/firmware/micro_speech_sim/micro_speech/recognize_commands.cpp
decisions:
  - No code changes; document findings and propose fixes in report.
actions:
  - Reviewed I2S test sketch configuration and audio capture path.
  - Checked inference loop flow and logging behavior.
results:
  - Found busy-wait in audio init, partial-read handling that can reuse stale audio, I2S port mismatch between test/inference, README fake-mic mismatch, and duplicate micro_speech source copies.
artifacts:
  - notes/Journal/agent/tasks/1500cd2d/report.md
tests:
  - Not run (review only).
next:
  - If approved, patch audio init wait + partial read handling, align I2S config and README, then collect ESP32 serial logs.
open_questions:
  - Which I2S port should be canonical for the target board (0 or 1)?


File: C:\Users\Danil\diploma_esp32_distributed_nn\notes\Journal\2026-02-24_agent_ouroboros_015.yaml
﻿date: 2026-02-24
session_id: agent_ouroboros_015
goal: Add quiet-hours guard for auto-scheduled tasks (after 02:00 MSK)
inputs:
  - user instruction: do not create new tasks after 02:00 MSK
actions:
  - added quiet_hours_msk config and gating in runner
  - added MSK time helpers in utils
  - updated BIBLE with quiet-hours rule
results:
  - evolution auto-scheduling is suppressed during MSK quiet hours
artifacts:
  ...
  - BIBLE.md
tests:
  - python -m py_compile code/agent/runner.py code/agent/utils.py
next:
  - confirm quiet-hours end time if different from 08:00 MSK
open_questions:
  - should manual /task be blocked during quiet hours too?
