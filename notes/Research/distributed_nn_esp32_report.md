---
title: "Распределенная сеть нейронных сетей на микроконтроллерах ESP32"
subtitle: "Исследовательский отчет и проектная проработка дипломной темы"
author: "Подготовлено для дипломного исследования"
date: "2026-02-16"
lang: "ru-RU"
toc: true
toc-depth: 3
...
# 2. Executive Summary (обновленная версия)
...
![Концепт универсальной архитектуры distributed neural fabric для кластера микроконтроллеров. Иллюстрация подготовлена в рамках этого отчета.](images/fig_distributed_neural_fabric.png)

---

# 3. Литературный обзор

## 3.1 База TinyML на MCU
...
## 3.2 Квантование, дистилляция, pruning как научная основа
...
## 3.3 Распределенный и split инференс
...
## 3.4 Бенчмарки и reproducibility
...
## 3.5 Аннотированный обзор ключевых статей (что именно брать в дипломный текст)
...
### [R12] Split-Learning TinyML on ESP32-S3
...
1. Требует аккуратной настройки температуры, весов loss и контроля overfitting.

### [R9] KWS pruning/quantization on low-power MCU
...
## 3.6 Литературные пробелы (где есть место для твоей новизны)
...
# 4. ESP32/ESP32-S3 для ML: тех-обзор «для чайника, но без упрощенчества»

## 4.1 Что важно понять сразу
...
## 4.2 ESP32 семейство (практический взгляд)
...
### ESP32-S3 (предпочтительный для диплома)
...
## 4.3 Память: внутренняя RAM vs PSRAM
...
## 4.5 Сеть и задержки: ESP-NOW/Wi-Fi
...
![ESP Wi-Fi datapath из документации ESP-IDF (официальный графический материал Espressif). Источник: [R22].](images/esp_wifi_datapath.png)
...
### Как устроен жизненный цикл программы на ESP32
...
### Что такое «ядра и пиннинг» в практическом смысле
...
1. очередь растет,
2. окно опаздывает,
3. итоговая accuracy в реальном времени падает даже при хорошей offline метрике.

### Почему offline accuracy может обмануть

В ноутбуке на GPU модель дает условно 96% accuracy. На MCU в системе могут появиться:
...
### Что такое «безопасный» путь к работающему прототипу
...
## 4.8 Практические ограничения ESP-NOW именно для ML-пакетов

По официальной документации [R21]:

1. v2 поддерживает payload до 1470 байт, v1 — до 250 байт.
2. Есть ограничения по числу peer.
...
## 4.9 Power-save и «скрытые» задержки
...
# 5. Твоя исходная идея («блоки сети на разных MCU») и ее критический разбор

## 5.1 В чем сильная сторона идеи
...
## 5.2 Главный риск: стоимость передачи активаций
...
2. Это часто хуже, чем доинференсить локально и отправить короткий вектор логитов.
...
![Зависимость «чистого» времени передачи от размера payload при разных эффективных скоростях канала. Иллюстрация построена по расчетной модели PHY time.](images/fig_comm_latency_tradeoff.png)

## 5.3 Когда блочная идея все же жизнеспособна
...
![Карта жизнеспособности layer-split в зависимости от размера промежуточного тензора и скорости канала. Значения в клетках — оценка времени одной передачи.](images/fig_layer_split_viability.png)

---

# 5.4 Как спасти исходную «блочную» идею и сделать ее научно сильной
...
# 5.5 Универсальный план эксперимента для CC-NBG (не только под аудио)
...
# 6. Кандидаты дипломных концептов (с критикой)

## 6.1 Кандидат A (рекомендуемый): CC-NBG как базовая архитектура, KWS как первый демонстратор
...
## 6.2 Кандидат B: Распределенное обнаружение бытовых аномалий (звук + вибрация + газ)
...
## 6.3 Кандидат C: Чистый layer-split DNN между ESP32
...
## 6.4 Кандидат D: Федеративная персонализация tiny-моделей на ESP32
...
## 6.5 Сравнительная матрица (формализованный выбор)
...
# 7. Лучшая идея: детальная проработка
...
\[
z_i = f_i(x_i; \theta_i), \quad p_i = \mathrm{softmax}(z_i)
\]
...
\[
w_i = \phi(\mathrm{SNR}_i, \mathrm{RSSI}_i, 1-u_i)
\]

Слияние (пример: взвешенный product/logit fusion):

\[
\hat{z} = \sum_{i=1}^{N} w_i z_i, \quad \hat{p}=\mathrm{softmax}(\hat{z})
\]
...
## 7.4 Routing policy (ядро новизны)

Пример политики:

1. Если \(u_i < \tau_1\), узел может отдать локальное решение (экономия трафика).
2. Если \(\tau_1 \le u_i < \tau_2\), узел инициирует кооперативный fusion.
3. Если \(u_i \ge \tau_2\), активируется second-stage (более тяжелый классификатор) только для сложных случаев.
...
## 7.5 Почему это лучше для ESP32 именно с учетом задержек

### Сравнение коммуникационной нагрузки
...
## 7.6 Предлагаемая двухуровневая архитектура (детально)
...
Для диплома здесь появляется удобная исследовательская ось: как выбор порогов \(\tau_1,\tau_2\), политики переключения и бюджета токена меняет одновременно accuracy, latency и энергопотребление. Именно эта совместная зависимость делает тему научной, а не просто инженерной.

## 7.7 Как обучать под distributed режим, а не просто «обычный KWS»
...
## 7.8 Анти-гипотезы, которые нужно честно проверять
...
1. Выигрыш в accuracy съедается ростом latency.
...
## 7.9 Memory budget: как быстро оценивать влезаемость модели

Приближенная оценка RAM на инференс:

\[
M_{total} \approx M_{weights} + M_{activations\_peak} + M_{runtime} + M_{buffers}
\]

Где:

1. \(M_{weights}\): размер квантованных весов (INT8 обычно 1 байт на вес).
2. \(M_{activations\_peak}\): пиковый размер промежуточных тензоров.
...
## 7.10 Почему ESP32-S3 предпочтительнее классического ESP32 для этой задачи
...
# 8. Экспериментальный дизайн (чтобы защита была сильной)

## 8.1 Аппаратный стенд
...
## 8.7 Протокол сценариев (чтобы результаты были валидны)
...
## 8.8 Как измерять энергию корректно
...
# 9. Инженерные детали реализации

## 9.1 Рекомендованная структура задач FreeRTOS
...
## 9.5 Рекомендуемая структура репозитория
...
## 9.6 Минимальный протокол логирования
...
## 9.7 Типичные embedded-баги в таких проектах

1. Непредсказуемые malloc/free в hot path.
2. Большие memcpy внутри callback.
...
# 10. Кодовый скелет (концептуально)
...
# 11. Где именно научная новизна (чтобы комиссия не сказала «это просто инженерка»)

## 11.1 Novelty 1: Communication-aware distillation

Teacher обучается централизованно, а student-узлы получают loss, штрафующий трафик:

\[
\mathcal{L}_{student} = \alpha \cdot CE(y, p_s) + \beta \cdot KL(p_t || p_s) + \gamma \cdot \mathrm{BitsPenalty}
\]

Где `BitsPenalty` зависит от частоты отправки и размера сообщений.

## 11.2 Novelty 2: Latency-aware adaptive routing

Политика выбора режима (`local-only`, `fusion`, `stage2`) оптимизирует:

\[
\max \; U = \mathrm{AccuracyGain} - \eta_1 \cdot T_{e2e} - \eta_2 \cdot E
\]
...
# 12. Слабые места проекта (критика заранее)

## 12.1 Риск синхронизации
...
# 13. Почему этот диплом реально может быть «лучше существующих решений»
...
## 13.1 Пример ожидаемой таблицы результатов (шаблон)
...
## 13.2 Что считать «успехом диплома»
...
# 14. Дорожная карта диплома (пример на 16 недель)
...
![Методологический конвейер исследования: от поиска разрезов и bottleneck-модулей до валидации latency/energy на железе.](images/fig_methodology_pipeline.png)

---

# 15. Практические рекомендации «что делать завтра»
...
# Приложение A. Быстрый глоссарий (ML -> embedded)
...
# Приложение B. Источники и ссылки

## B.1 TinyML, distributed inference, optimization
...
## B.2 ESP32 official documentation and tooling
...
## B.3 Additional ecosystem references
...
# Приложение C. Что еще можно добавить в финальную версию диплома
...
### [R4] On-device training under 256KB
...
## D.2 Компрессия, квантование, дистилляция
...
### [R8] Low precision quantization for TinyML
...
### [R9] KWS pruning + quantization на low-power MCU
...
## D.3 Distributed/split inference
...
### [R12] Split-Learning TinyML on ultra-low-power nodes
...
## D.5 Официальные источники Espressif (обязательны в дипломе)
...
### [R22] Wi-Fi performance/power-save
...
# Приложение E. Сильные и слабые аргументы на защите

## E.1 Сильные аргументы
...
## E.2 Слабые аргументы (которые лучше не использовать)
...
## E.3 Что спросит комиссия и как отвечать
...
# Приложение F. Развернутые обзоры статей (сплошной текст)
...
## F.3 Distributed и split inference: эволюция идей
...
## F.4 Бенчмарки, датасеты и прикладные ориентиры
...
# Приложение G. Финальная архитектурная позиция после критики
...
## G.1 Конкретный критерий жизнеспособности split с энкодером/декодером
...
\]
\[
\Delta Acc \le \epsilon,
\]
\[
B_{token} \le B_{budget}.
\]
...
## G.2 Практическое решение для диплома: гибридный двухконтурный режим

Финальная рекомендуемая архитектура следующая. Первый контур — MoE-fast path. Узлы-специалисты дают локальные логиты/оценки, координатор делает routing и fusion. Этот контур устойчив, прост в отладке и хорошо переносит packet loss, потому что каждый узел — законченный инференс-эксперт, а не половина чужого слоя.
...
## G.3 Что именно будет научной новизной в такой финальной версии
...
## G.4 Жесткая практическая конкретика
